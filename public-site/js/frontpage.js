...
(function () {
  ...
  function loadSummary() {
    Promise.all([
      safeFetch('/api/public/summary'),
      safeFetch('/api/health/charge-ratio')
    ]).then(function (responses) {
      var data = responses[0];
      var chargeData = responses[1];
      if (!data) return;
      ...
      if (chargeData && typeof chargeData.chargeRatio === 'number') {
        el('s-charge').textContent = Math.round(chargeData.chargeRatio * 100) + '%';
        el('s-charge').title = 'Status: ' + String(chargeData.status || 'unknown');
      } else {
        el('s-charge').textContent = '--';
      }
  ...
  function loadLeaderboard() {
    safeFetch('/api/public/leaderboard?limit=5').then(function (data) {
      ...
      if (!data || !Array.isArray(data) || data.length === 0) {
        container.innerHTML = '<div class="warming-up"><span class="spinner"></span> Het systeem is actief en genereert de eerste rapporten. Kom over enkele uren terug.</div>';
        return;
      }
      ...
      container.innerHTML =
        ...
        '<tbody>' + rows + '</tbody>' +
        '</table>' +
        '<div style="margin-top:12px"><a href="/leaderboard" class="btn btn-secondary" style="font-size:11px;padding:8px 20px">VIEW TOP GAPS</a></div>';
    });
  }
