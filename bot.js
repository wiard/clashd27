 ...
 * Thin Discord wrapper around TickEngine.
 */
...
const TICK_INTERVAL = parseInt(process.env.TICK_INTERVAL || '300') * 1000;
...
// ---------------------------------------------------------------------------
// TickEngine + event listeners
// ---------------------------------------------------------------------------
const engine = new TickEngine({ state });

engine.on('tick', async ({ tickNum, activeCell, cycle, events, isCycleEnd }) => {
  ...
  if (events.length > 0) {
    ...
    if (cellEvents.length > 0) {
      const ce = eventEmbed(cellEvents);
      if (ce) await sendToChannel(`${CHANNEL_PREFIX}${activeCell}`, { embeds: [ce] });
    }
...
engine.on('cycleEnd', async ({ summary }) => {
  await sendToChannel('residue', { embeds: [cycleEmbed(summary)] });
...
client.once('ready', async () => {
  ...
  if (process.env.USE_CUBE === 'true') {
    ...
    // Retraction enricher (background init)
    try {
...
process.on('SIGINT', () => { engine.stop(); client.destroy(); process.exit(0); });
process.on('SIGTERM', () => { engine.stop(); client.destroy(); process.exit(0); });
