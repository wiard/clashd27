...
function isRecognizedAgency(agency) {
  ...
  const lower = agency.split(':')[0].toLowerCase().trim();
  if (lower.startsWith('national institute of') || lower.startsWith('national center for')) return true;
  if (lower.startsWith('cancer prevention and research institute')) return true;
...
function classifyAgencyRegion(agency) {
  ...
  if (lower.startsWith('national institute of') || lower.startsWith('national center for')) return 'us';
  if (lower.startsWith('cancer prevention and research institute')) return 'us';
...
async function enrichWithFunding(gapPacket) {
  ...
  // Ensure hasRecognizedFunding is set (may be missing from cached data)
  for (const p of papers) {
    p.hasRecognizedFunding = (p.grantsList || []).some(g => isRecognizedAgency(g.agency));
  }
