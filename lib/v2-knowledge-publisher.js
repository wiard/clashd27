...
function buildGapHandoffDedupeKey(handoff) {
  ...
  return `gap_handoff:gap_api_v1:${packetId}`;
}
...
function buildGapApiHandoffPayload(handoff) {
  if (!handoff || typeof handoff !== 'object') {
    return null;
  }

  const packet = handoff.packet && typeof handoff.packet === 'object' ? handoff.packet : null;
  if (!packet) {
    return null;
  }

  return {
    kind: 'gap_proposal_handoff',
    source: typeof handoff.source === 'string' && handoff.source ? handoff.source : 'clashd27',
    handedOffAtIso: typeof handoff.handedOffAtIso === 'string' && handoff.handedOffAtIso
      ? handoff.handedOffAtIso
      : (typeof handoff.generatedAt === 'string' && handoff.generatedAt ? handoff.generatedAt : new Date().toISOString()),
    packet
  };
}
...
async function publishGapProposalHandoff(handoff, options = {}) {
  ...
  const payload = buildGapApiHandoffPayload(handoff);
  if (!proposal || !payload || !handoff || typeof handoff !== 'object') {
  ...
  return withGapHandoffStoreLock(async () => {
    ...
    const url = `${gatewayUrl.replace(/\/$/, '')}/api/gaps/propose?token=${encodeURIComponent(token)}`;
    try {
      console.log('[GOVERNED] POST handoff:', JSON.stringify({
        packetId: handoff.packetId,
        url
      }));
      const response = await fetchImpl(url, {
      ...
      console.log('[GOVERNED] POST response:', JSON.stringify({
        packetId: handoff.packetId,
      ...
      const text = await response.text().catch(() => '');
      console.error('[GOVERNED] POST failure body:', JSON.stringify({
        packetId: handoff.packetId,
        status: response.status,
      ...
      console.error('[GOVERNED] POST exception:', JSON.stringify({
        packetId: handoff.packetId,
        message: err.message
      }));
      appendGapHandoffRecord(store, {
...
async function publishGapProposalHandoffs(handoffs, options = {}) {
  ...
  for (const handoff of handoffs || []) {
    let result = await publishGapProposalHandoff(handoff, options);
    if (result.failed && result.status === 403 && typeof result.reason === 'string' && result.reason.includes('contract.rate_limited')) {
      await sleep(10_500);
      result = await publishGapProposalHandoff(handoff, options);
    }
    summary.results.push(result);
    if (result.published) summary.published += 1;
    else if (result.deduped) summary.deduped += 1;
