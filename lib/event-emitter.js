...
function runDiscoveryCycle(engine, opts = {}) {
  ...
  const systemEntropy = opts.systemEntropy || readLatestSystemEntropy();
...
function readLatestSystemEntropy() {
  const targetPath = process.env.OPENCLASHD_SYSTEM_ENTROPY_PATH ||
    path.join(os.homedir(), '.openclashd', 'system-entropy.jsonl');
  try {
