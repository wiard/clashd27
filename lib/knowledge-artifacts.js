...
function persistKnowledgeArtifact(discovery, dataDir) {
  ...
  // Optional linkable identifiers for downstream graph systems
  if (discovery.parentCandidateId) {
    artifact.parentCandidateId = discovery.parentCandidateId;
  }
  if (Array.isArray(discovery.relatedCandidateIds) && discovery.relatedCandidateIds.length > 0) {
    artifact.relatedCandidateIds = discovery.relatedCandidateIds;
  }
  if (typeof discovery.originatingTick === 'number') {
    artifact.originatingTick = discovery.originatingTick;
  }
  if (discovery.originatingClusterId) {
    artifact.originatingClusterId = discovery.originatingClusterId;
  }
  if (discovery.graphHints && typeof discovery.graphHints === 'object') {
    artifact.graphHints = discovery.graphHints;
  }
