...
function classifySurprise(paper) {
  ...
  // Check anomalous markers (+3 each)
  ...
  // Citation burst: high citations for recent paper
  const currentYear = new Date().getFullYear();
  const age = currentYear - (paper.year || currentYear);
  if (paper.citationCount > 50 && age <= 2 && age >= 0) {
    score += 2; // citation burst bonus
  }

  // Influential citation ratio bonus
  ...
  // Map score to 0/1/2
  if (score >= 4) return 2; // anomalous
  if (score >= 1) return 1; // deviation
