...
function detectDiscoveryCandidates(input) {
  const { gravityCells, emergenceSummary, cubeState, systemEntropy } = input;
  ...
  for (const col of collisions) {
    ...
    const entropyWeight = clamp(Number(col.entropyWeight || 0), 0, 1);
    const systemOrderBonus = computeSystemOrderBonus(systemEntropy, 'collision');

    // V2 ranking formula
    const candidateScore = round(Math.min(1,
      (col.emergenceScore * 0.33) +
      (clamp(combinedGravity / 2, 0, 1) * 0.22) +
      (domDist / 0.3 * 0.18) +
      (evidDensity * 0.10) +
      (srcConf * 0.10) +
      (entropyWeight * 0.07) +
    ...
    candidates.push({
      ...
      rankingMetadata: {
        ...
        entropyWeight,
        systemEntropyScore: safeUnitNumber(systemEntropy && systemEntropy.entropy_score),
  ...
  for (const cluster of clusters) {
    ...
    const entropyClusterScore = clamp(Number(cluster.entropyClusterScore || 0), 0, 1);
    const systemOrderBonus = computeSystemOrderBonus(systemEntropy, 'cluster');
    ...
    const candidateScore = round(Math.min(1,
      (clamp(cluster.totalScore / cluster.size, 0, 1) * 0.32) +
      (clamp(gravityScore / 3, 0, 1) * 0.22) +
      (maxDomDist / 0.3 * 0.18) +
      (evidDensity * 0.10) +
      (srcConf * 0.10) +
      (entropyClusterScore * 0.08) +
      systemOrderBonus
    ));
  ...
  for (const gradient of gradients) {
    ...
    const systemOrderBonus = computeSystemOrderBonus(systemEntropy, 'gradient');
...
function computeSystemOrderBonus(systemEntropy, candidateType) {
  const entropyScore = safeUnitNumber(systemEntropy && systemEntropy.entropy_score);
  const orderScore = 1 - entropyScore;
  const typeMultiplier = candidateType === 'cluster' ? 1 : candidateType === 'collision' ? 0.9 : 0.75;
  return round(orderScore * 0.06 * typeMultiplier);
}
