...
function computeNoveltyScore(cells, cubeState) {
  // Based on time axis: emerging=0.9, current=0.6, historical=0.3
...
function computeEvidenceDensity(cells, cubeState) {
  // Based on unique source count / max expected
  const allCells = cubeState.cells || {};
  ...
  return round(Math.min(1, sources.size / 4));
}

function computeSourceConfidence(sources) {
  // Based on source type diversity
  ...
  for (const col of collisions) {
    ...
    candidates.push({
      ...
      rankingMetadata: {
        ...
        evidenceDensity: computeEvidenceDensity(colCells, cubeState),
        crossDomainScore: crossDomain ? 0.8 : 0.3,
        candidateType: 'collision_intersection',
        sourceConfidence: computeSourceConfidence(col.sources),
        governanceValue: 0.6
      }
    });
  }
  ...
  for (const cluster of clusters) {
    ...
    candidates.push({
      ...
      rankingMetadata: {
        ...
        evidenceDensity: computeEvidenceDensity([strongId], cubeState),
        crossDomainScore: diverseSources ? 0.6 : 0.3,
        candidateType: 'cluster_peak',
        sourceConfidence: computeSourceConfidence(cell.uniqueSourceTypes || []),
        governanceValue: 0.5
      }
    });
  }
  ...
  for (const gradient of gradients) {
    ...
    candidates.push({
      ...
      rankingMetadata: {
        ...
        evidenceDensity: computeEvidenceDensity(path, cubeState),
        crossDomainScore: spansDomains ? 0.7 : 0.3,
