...
const { getNeighbors, getNeighborType, isCrossLayer, NEIGHBOR_TYPE } = require('./cube');
...
function logAgentEvent(event) {
  try {
    ...
    const tmpH = HISTORY_FILE + '.tmp';
    fs.writeFileSync(tmpH, JSON.stringify(history, null, 2));
    fs.renameSync(tmpH, HISTORY_FILE);
  } catch (e) {
...
function getCellLabel(cell) {
  // Cube mode: use dynamic paper-based cell descriptions
  if (process.env.USE_CUBE === 'true') {
    try {
      const { getCubeDescription } = require('./shuffler');
      const desc = getCubeDescription(cell);
      if (desc) return desc.description;
    } catch (e) {
      // Shuffler not ready yet — fall through to pack
    }
  }
  // Pack mode: use static domain labels
