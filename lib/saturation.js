...
async function checkSaturation(hypothesis, abc_chain, bridge) {
  ...
  // Pre-fetch S2 paper counts to ground the LLM's estimate
  let s2PaperHint = '';
  try {
    const shortHyp = hypothesis.replace(/[^a-zA-Z0-9\s]/g, ' ').trim().slice(0, 80);
    if (shortHyp.length > 10) {
      const papers = await searchPapers(shortHyp, 5);
      if (papers.length > 0) {
        const totalEstimate = papers[0]._searchTotal || papers.length;
        s2PaperHint = `\nNOTE: Semantic Scholar returned ~${totalEstimate} results for this topic (top result: "${papers[0].title}", ${papers[0].year}, ${papers[0].citationCount} citations). Use this as a calibration point for your paper_estimate_5y.`;
        console.log(`[SATURATION] S2 pre-check: ~${totalEstimate} results for "${shortHyp.slice(0, 50)}..."`);
      }
    }
  } catch (e) {
    console.error(`[SATURATION] S2 pre-check failed (non-fatal): ${e.message}`);
  }
...
Return JSON only:
{
  ...
  try {
    ...
    if (!budget.canAffordCall(MODEL, estimatedInputTokens, 1000)) {
      console.log(`[BUDGET] Skipping saturation check (model=${MODEL}) — daily budget reached`);
      return null;
    }
    ...
    trackCost(MODEL, estimatedInputTokens, budget.estimateTokens(text));
...
function trackCost(model, inputTokens = 0, outputTokens = 0) {
  try {
    let metrics = {};
