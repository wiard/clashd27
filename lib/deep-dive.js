...
const DEEP_DIVE_DELAY_MS = Math.max(0, parseInt(process.env.DEEP_DIVE_DELAY_MS || '5000', 10));
...
function saveDeepDive(dive) {
  ...
  const tmpFile = DEEP_DIVES_FILE + '.tmp';
  fs.writeFileSync(tmpFile, JSON.stringify(data, null, 2));
  fs.renameSync(tmpFile, DEEP_DIVES_FILE);
  return dive;
}
...
async function callWithSearch(prompt, maxTokens = 1500) {
  ...
  if (!budget.canAffordCall(MODEL, estimatedInputTokens, maxTokens)) {
    console.log(`[BUDGET] Skipping deep-dive call (model=${MODEL}) — daily budget reached`);
    return null;
  }
  try {
    ...
    if (!response.ok) {
      ...
      console.log('[DEEP-DIVE] Falling back to direct Anthropic call');
...
async function callAnthropicDirect(prompt, maxTokens) {
  if (!ALLOW_DIRECT_ANTHROPIC_FALLBACK) {
    console.log('[DEEP-DIVE] Direct Anthropic fallback disabled — skipping fallback call');
    return null;
  }
  ...
  try {
    ...
    budget.trackCall(MODEL, budget.estimateTokens(prompt), budget.estimateTokens(text));
    return text || null;
  } catch (err) {
    console.error(`[DEEP-DIVE] Direct Anthropic fallback failed: ${err.message}`);
    return null;
  }
}
...
Maximum 3 labs. Be specific about real institutions and researchers.`;
  ...
  if (DEEP_DIVE_DELAY_MS > 0) {
    console.log(`[DEEP-DIVE] Waiting ${DEEP_DIVE_DELAY_MS}ms for rate limit...`);
