...
const { recordDailyCandidate } = require('./gap-publisher');
...
const DEFAULT_MODEL = DEEP_MODEL;
const ALLOW_DIRECT_ANTHROPIC_FALLBACK = process.env.CLASHD27_ALLOW_DIRECT_ANTHROPIC_FALLBACK === 'true';
...
const DOMAIN_SESSION_CONFIGS = {
  'cancer-research': {
    packId: 'cancer-research',
...
async function callAnthropicDirect(systemPrompt, userPrompt, maxTokens, model) {
  if (!ALLOW_DIRECT_ANTHROPIC_FALLBACK) {
    console.log('[RESEARCHER] Direct Anthropic fallback disabled — skipping fallback call');
    return null;
  }
  ...
  try {
    ...
    const estimatedOutputTokens = budget.estimateTokens(text);
    budget.trackCall(model, estimatedInputTokens, estimatedOutputTokens);
    trackApiCall(model, estimatedInputTokens, estimatedOutputTokens);
    return text;
  } catch (err) {
    console.error(`[RESEARCHER] Direct Anthropic fallback failed: ${err.message}`);
    return null;
  }
}
 ...
 */
async function callWithSearch(prompt, maxTokens = 1000, model = DEFAULT_MODEL) {
  if (isCircuitOpen()) return null;
  const estimatedInputTokens = budget.estimateTokens(prompt);
  if (!budget.canAffordCall(model, estimatedInputTokens, maxTokens)) {
    console.log(`[BUDGET] Skipping call (model=${model}) — daily budget reached`);
    return null;
  }
  try {
    ...
    if (!response.ok) {
      ...
      console.log('[RESEARCHER] Falling back to direct Anthropic call');
      return callAnthropicDirect(null, prompt, maxTokens, model);
    }
    ...
    // Track cost
    const outputTokens = budget.estimateTokens(text);
    budget.trackCall(model, estimatedInputTokens, outputTokens);
    trackApiCall(model, estimatedInputTokens, outputTokens);
 ...
 */
async function callWithSystemPrompt(systemPrompt, userPrompt, maxTokens = 1500, model = DEFAULT_MODEL) {
  if (isCircuitOpen()) return null;
  const estimatedInputTokens = budget.estimateTokens(systemPrompt) + budget.estimateTokens(userPrompt);
  if (!budget.canAffordCall(model, estimatedInputTokens, maxTokens)) {
    console.log(`[BUDGET] Skipping call (model=${model}) — daily budget reached`);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`[RESEARCHER] API HTTP error: ${response.status} — ${errBody.substring(0, 300)}`);
      console.log('[RESEARCHER] Falling back to direct Anthropic call');
      return callAnthropicDirect(systemPrompt, userPrompt, maxTokens, model);
    }
...
function saveFinding(finding) {
  ...
  if (finding.domain) {
    finding.domain = String(finding.domain);
  }
  ...
  const tmpFile = FINDINGS_FILE + '.tmp';
 ...
 */
async function investigateCell({ tick, agentName, cell, cellLabel, packName, domain }) {
  ...
  const prompt = `You are a research agent investigating "${cellLabel}" in the domain of ${packName}. Search the web for the most significant recent developments (last 30 days) in this specific area. Find REAL papers, REAL clinical results, REAL data. Return JSON only, no markdown:
{
  ...
  const rawText = await callWithSearch(prompt, 1000, SCREENING_MODEL);
 ...
 */
async function investigateBond({ tick, cell, cellLabel, agent1Name, agent2Name, cell1Label, cell2Label, packName, cell1Keywords, cell2Keywords, domain }) {
 ...
 */
async function investigateDiscovery({ tick, cell, cellLabel, agent1Name, agent2Name, layer0Cell, layer2Cell, packName, dataKeywords, hypothesisKeywords, priorSummary, meetingPoint: meetingPointOverride, cubeMode, goldenCollision, rubric, scoringVersion, skipScreening = false, domain }) {
  const isCube = cubeMode && typeof layer0Cell === 'object';
  const logLabel = isCube
    ? `[${layer0Cell.methodLabel}/${layer0Cell.surpriseLabel}] x [${layer2Cell.methodLabel}/${layer2Cell.surpriseLabel}]`
    : `${layer0Cell} (data) x ${layer2Cell} (hypothesis)`;
  console.log(`[RESEARCHER] DISCOVERY investigation${isCube ? ' [CUBE]' : ''}: ${logLabel}`);
  ...
  // ── Semantic Scholar pre-fetch (pack mode only; cube mode has papers embedded) ──
  ...
  const systemPrompt = 'You identify cross-domain research gaps. Be concise. Return strict JSON only.';

  const meetingPoint = meetingPointOverride || cellLabel || '';
  const rubricClause = rubric ? `\nLENS: ${rubric.label}\n${rubric.prompt}\n` : '';

  // ── Build user prompt: cube mode vs pack mode ──
  let userPrompt;

  if (isCube) {
    // Cube mode: structured cell descriptions with actual papers + source indicators
    const formatCellPapers = (cellDesc) => {
      if (!cellDesc.topPapers || cellDesc.topPapers.length === 0) return '  (no papers in this cell)';
      return cellDesc.topPapers.map(p => {
        const sourceTag = p.source ? ` [${p.source}]` : '';
        const retractedTag = p.isRetracted ? ' ⚠️RETRACTED' : '';
        return `  - "${p.title}" (${p.year || '?'}, ${p.citationCount || 0} cites)${p.doi ? ` DOI:${p.doi}` : ''}${sourceTag}${retractedTag}\n    ${(p.abstract || '').slice(0, 200)}`;
      }).join('\n');
    };

    const goldenClause = (goldenCollision && goldenCollision.score > 0)
      ? `\n--- COLLISION SCORING ---
Method distance: ${goldenCollision.components.methodDistance} | Surprise pair: ${goldenCollision.components.surprisePair} | Semantic distance: ${goldenCollision.components.semanticDistance}
Collision score: ${goldenCollision.score}${goldenCollision.golden ? ' ★ GOLDEN COLLISION — these clusters are maximally different. Look for unexpected bridges between different methodologies and surprising findings.' : ''}
`
      : '';

    // v2.0: Compute dormant gap context (best-effort, non-blocking)
    let dormantGapClause = '';
    try {
      const papersA = (layer0Cell.topPapers || []).filter(p => p.doi);
      const papersB = (layer2Cell.topPapers || []).filter(p => p.doi);
      if (papersA.length > 0 && papersB.length > 0) {
        // Check for shared references between top papers (lightweight)
        const doisA = papersA.map(p => p.doi).slice(0, 2);
        const doisB = papersB.map(p => p.doi).slice(0, 2);
        dormantGapClause = `\n--- CROSS-CITATION CONTEXT ---
Cluster A DOIs: ${doisA.join(', ')}
Cluster B DOIs: ${doisB.join(', ')}
If these clusters have zero cross-citations but shared references, this is a DORMANT GAP — the highest-value discovery target.
`;
      }
    } catch (e) {
      // non-fatal
    ...
    userPrompt = `Two research clusters collide:

CLUSTER A [cell ${layer0Cell.cell}] — Method: ${layer0Cell.methodLabel} | Surprise: ${layer0Cell.surpriseLabel} (avg ${layer0Cell.surpriseScoreAvg || 0}) | Semantic: ${layer0Cell.clusterLabel} (${layer0Cell.paperCount} papers)
Representative papers:
${formatCellPapers(layer0Cell)}

CLUSTER B [cell ${layer2Cell.cell}] — Method: ${layer2Cell.methodLabel} | Surprise: ${layer2Cell.surpriseLabel} (avg ${layer2Cell.surpriseScoreAvg || 0}) | Semantic: ${layer2Cell.clusterLabel} (${layer2Cell.paperCount} papers)
Representative papers:
${formatCellPapers(layer2Cell)}

Meeting point: ${meetingPoint}
${rubricClause}${goldenClause}${dormantGapClause}${priorClause}

Find cross-domain connections between these paper clusters. Look for:
- Shared entities (proteins, genes, molecules, mathematical patterns) studied with DIFFERENT methods
- Anomalous results in one cluster that could be explained by findings in the other
- Methodological bridges: a technique from cluster A never applied to the subject matter in cluster B
- DORMANT GAPS: if both clusters cite common references but never cite each other, that's a high-value gap

If you cannot find strong evidence for at least 2 links in the A-B-C chain, return: {"no_gap": true, "reason": "your explanation"} instead.

Otherwise produce a GAP PACKET in this exact JSON format:`;
  } else {
    // Pack mode: original domain-based prompt
    userPrompt = `Three research domains intersect:
...
Otherwise produce a GAP PACKET in this exact JSON format:`;
  ...
  // Append the gap packet JSON schema (shared between both modes)
  userPrompt += `
...
- MAX ONE inferential leap: at most one abc_chain link may be weak or moderate.
...
Return ONLY valid JSON. No markdown, no explanation.`;
  ...
  function buildCleanedPacket(parsedPacket) {
    return {
      hypothesis: clean(parsedPacket.hypothesis),
      abc_chain: cleanChain(parsedPacket.abc_chain),
      bridge: {
        claim: clean((parsedPacket.bridge || {}).claim || ''),
        status: (parsedPacket.bridge || {}).status || 'speculative',
        required_evidence: clean((parsedPacket.bridge || {}).required_evidence || '')
      },
      supporting_sources: cleanArr(parsedPacket.supporting_sources).slice(0, 3),
      limiting_sources: cleanArr(parsedPacket.limiting_sources).slice(0, 3),
      kill_test: clean(parsedPacket.kill_test || ''),
      cheapest_validation: {
        method: clean((parsedPacket.cheapest_validation || {}).method || ''),
        required_data: cleanArr((parsedPacket.cheapest_validation || {}).required_data),
        statistical_approach: clean((parsedPacket.cheapest_validation || {}).statistical_approach || ''),
        estimated_time: clean((parsedPacket.cheapest_validation || {}).estimated_time || '')
      },
      clinical_relevance: clean(parsedPacket.clinical_relevance || ''),
      impact: parsedPacket.impact || 'medium',
      feasibility: parsedPacket.feasibility || 'medium'
    };
  }

  if (!skipScreening) {
    // --- Screening: Haiku first, Sonnet only for promising gaps ---
    const screeningPrompt = `${userPrompt}\n\nSCREENING: Be concise. If clearly no gap, return {\"no_gap\": true, \"reason\": \"...\"}.`;
    const screeningText = await callWithSystemPrompt(systemPrompt, screeningPrompt, 1500, SCREENING_MODEL);
    const screeningParsed = parseJSON(screeningText);

    if (!screeningParsed) {
      console.log(`[RESEARCHER] Discovery screening returned no parseable result`);
      return null;
    }

    if (screeningParsed.no_gap === true) {
      const noGapLabel = isCube ? logLabel : `${layer0Cell} × ${layer2Cell}`;
      console.log(`[RESEARCH] NO GAP (screening): ${noGapLabel} — ${screeningParsed.reason || 'insufficient evidence'}`);
      return { type: 'no_gap', reason: screeningParsed.reason || 'insufficient evidence', layer0Cell: isCube ? layer0Cell.description : layer0Cell, layer2Cell: isCube ? layer2Cell.description : layer2Cell, cubeMode: isCube };
    }

    const screeningValidation = validateGapPacket(screeningParsed);
    const screeningPacket = buildCleanedPacket(screeningParsed);
    const screeningRedFlags = checkRedFlags(screeningPacket);
    const screeningSpec = speculationIndex(screeningPacket);
    const screeningScores = scoreGapPacket(screeningPacket, screeningSpec);
    if (!screeningValidation.valid || screeningRedFlags.length > 0 || screeningScores.total < 75) {
      const noGapLabel = isCube ? logLabel : `${layer0Cell} × ${layer2Cell}`;
      console.log(`[RESEARCH] NO GAP (screening score ${screeningScores.total}): ${noGapLabel}`);
      return { type: 'no_gap', reason: `screening_score_${screeningScores.total}`, layer0Cell: isCube ? layer0Cell.description : layer0Cell, layer2Cell: isCube ? layer2Cell.description : layer2Cell, cubeMode: isCube };
    }
  }

  // Use system+user prompt format via the proxy (Sonnet deep pass)
  const rawText = await callWithSystemPrompt(systemPrompt, userPrompt, 2500, DEEP_MODEL);
  const parsed = parseJSON(rawText);
  ...
  if (parsed.no_gap === true) {
    const noGapLabel = isCube ? logLabel : `${layer0Cell} × ${layer2Cell}`;
    console.log(`[RESEARCH] NO GAP: ${noGapLabel} — ${parsed.reason || 'insufficient evidence for bridge'}`);
    return { type: 'no_gap', reason: parsed.reason || 'insufficient evidence', layer0Cell: isCube ? layer0Cell.description : layer0Cell, layer2Cell: isCube ? layer2Cell.description : layer2Cell, cubeMode: isCube };
  }
  ...
  const cleanedPacket = buildCleanedPacket(parsed);
  ...
  // Build cell labels for storage (strings, not objects)
  const storedCellLabels = isCube
    ? [layer0Cell.description, layer2Cell.description]
    : [layer0Cell, layer2Cell];
  ...
  const finding = saveFinding({
    ...
    cell,
    cells: [cell, cell],
    cellLabels: storedCellLabels,
    pack: packName,
    cubeMode: isCube || false,
    cubeMetadata: isCube ? {
      cellA: { cell: layer0Cell.cell, x: layer0Cell.x, y: layer0Cell.y, z: layer0Cell.z, method: layer0Cell.methodLabel, surprise: layer0Cell.surpriseLabel, cluster: layer0Cell.clusterLabel },
      cellB: { cell: layer2Cell.cell, x: layer2Cell.x, y: layer2Cell.y, z: layer2Cell.z, method: layer2Cell.methodLabel, surprise: layer2Cell.surpriseLabel, cluster: layer2Cell.clusterLabel }
    } : null,
  ...
  const finalLogLabel = isCube ? logLabel : `${layer0Cell} x ${layer2Cell}`;
  console.log(`[RESEARCHER] ${finding.id} | ${type.toUpperCase()}${isCube ? ' [CUBE]' : ''} | ${finalLogLabel} | verdict=${verdict} | impact=${parsed.impact} | feasibility=${parsed.feasibility} | novelty=${parsed.novelty}`);
  return finding;
}
 ...
 */
async function investigateFollowUp({ tick, agentName, cell, cellLabel, packName, followUp, domain }) {
 ...
 */
async function investigateVerification({ tick, agentName, cell, cellLabel, packName, claim, discoveryId, domain }) {
 ...
 * Track API call cost
 */
function trackApiCall(model, inputTokens = 0, outputTokens = 0) {
  try {
    let metrics = {};
    ...
    const cost = budget.estimateCostUSD(model, inputTokens, outputTokens);
    metrics.estimated_cost_total = Math.round(((metrics.estimated_cost_total || 0) + cost) * 100) / 100;
    metrics.estimated_cost_today = Math.round(((metrics.estimated_cost_today || 0) + cost) * 100) / 100;
...
async function runResearchSession({ domain = 'cancer-research', maxFindings = 20, keywords = [] } = {}) {
  const sessionConfig = DOMAIN_SESSION_CONFIGS[domain] || {};
  const pack = loadPack(sessionConfig.packId || domain);
  const packName = (pack && pack.name) || sessionConfig.label || domain;
  const activeKeywords = normalizeKeywords(
    Array.isArray(keywords) && keywords.length > 0 ? keywords : sessionConfig.keywords
  );

  if (activeKeywords.length < 2) {
    throw new Error(`Need at least 2 keywords to run research session for ${domain}`);
  }

  const boundedLimit = Math.max(1, Math.min(20, Number(maxFindings) || 20));
  const keywordPairs = buildKeywordPairs(activeKeywords, boundedLimit);
  const findings = [];
  const noGapPairs = [];

  console.log(`[RESEARCH-SESSION] domain=${domain} pack=${packName} keywords=${activeKeywords.length} maxFindings=${boundedLimit}`);

  for (let index = 0; index < keywordPairs.length; index += 1) {
    const [dataDomain, hypothesisDomain] = keywordPairs[index];
    const secondaryKeywords = activeKeywords.filter((keyword) => keyword !== dataDomain && keyword !== hypothesisDomain);

    const result = await investigateDiscovery({
      tick: Date.now() + index,
      cell: 18 + (index % 9),
      cellLabel: hypothesisDomain,
      agent1Name: `ResearchSession-${domain}-A`,
      agent2Name: `ResearchSession-${domain}-B`,
      domain,
      layer0Cell: dataDomain,
      layer2Cell: hypothesisDomain,
      packName,
      dataKeywords: [dataDomain, ...secondaryKeywords.slice(0, 2)],
      hypothesisKeywords: [hypothesisDomain, ...secondaryKeywords.slice(2, 4)]
    });
    ...
    if (result.type === 'no_gap') {
      noGapPairs.push({
        pair: [dataDomain, hypothesisDomain],
        reason: result.reason || 'insufficient evidence'
      });
      continue;
    }

    if (result.type === 'discovery' && Number(result.scores && result.scores.total) > 0) {
      recordDailyCandidate(result, Number(result.scores.total));
    }

    findings.push({
      id: result.id,
      type: result.type,
      verdict: (result.verdict && result.verdict.verdict) || result.verdict || null,
      score: Number(result.scores && result.scores.total) || 0,
      hypothesis: result.hypothesis || result.discovery || '',
      pair: [dataDomain, hypothesisDomain]
    });

    if (findings.length >= boundedLimit) {
      break;
    }
  }
  ...
  return {
    ...
    attemptedPairs: keywordPairs.length,
    findingsCreated: findings.length,
    discoveries: findings.filter((finding) => finding.type === 'discovery').length,
    drafts: findings.filter((finding) => finding.type === 'draft').length,
    duplicates: findings.filter((finding) => finding.type === 'duplicate').length,
    noGapPairs: noGapPairs.length,
