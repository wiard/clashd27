...
async function investigateBond({ tick, cell, cellLabel, agent1Name, agent2Name, cell1Label, cell2Label, packName, cell1Keywords, cell2Keywords }) {
  ...
  const prompt = `Two research agents have collided. Agent A has been investigating "${cell1Label}" and found keywords: [${(cell1Keywords || []).join(', ')}]. Agent B has been investigating "${cell2Label}" and found keywords: [${(cell2Keywords || []).join(', ')}]. Both work in ${packName}.

Search the web for REAL connections between these two areas. Look for:
- Papers that cite both topics
- Shared mechanisms or pathways
- A finding in one area that directly impacts the other
- Researchers who work at this exact intersection

Return JSON only, no markdown:
{
  "connection": "3-4 sentences describing the specific real connection you found between these areas. Names, mechanisms, evidence.",
  "source": "The actual paper, review, or article that demonstrates this connection",
  "date": "Publication date",
  "strength": "strong/moderate/weak — how direct is this connection?",
  "novelty": "high/medium/low — would specialists in both fields likely know this?",
  "hypothesis": "One testable hypothesis that emerges from this connection",
  "next_step": "One concrete next research action this suggests"
}`;
  ...
  console.log(`[RESEARCHER] ${finding.id} | BOND | ${cell1Label} x ${cell2Label} | ${parsed.strength} | ${parsed.novelty} novelty`);
  return finding;
}
