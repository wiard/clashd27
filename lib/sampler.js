 ...
 *   60% OpenAlex random (diverse fields)
 *   15% bioRxiv/medRxiv/arXiv fresh preprints
 *   10% OpenAlex targeted anomaly sampling
 *   10% S2 keyword queries (legacy, method/surprise biased)
...
const METHOD_QUERIES = [
  'MRI imaging tumor detection', 'histopathology cancer diagnosis',
  'fluorescence microscopy cell', 'mass spectrometry protein',
  'machine learning cancer prediction', 'bioinformatics genomic analysis',
  'deep learning medical imaging', 'computational drug design',
  'CRISPR gene knockout cancer', 'xenograft mouse model tumor',
  'clinical trial randomized cancer', 'in vitro drug screening'
];

const SURPRISE_QUERIES = [
  'unexpected finding cancer', 'paradoxical effect treatment',
  'contradicts previous findings', 'surprising result tumor',
  'failed to replicate cancer', 'novel mechanism disease',
  'counterintuitive immune response', 'anomalous results clinical'
];

const CROSSDOMAIN_QUERIES = [
  'microbiome immunotherapy response', 'artificial intelligence drug resistance',
  'epigenetics environmental exposure cancer', 'metabolomics cancer biomarker',
  'nanotechnology drug delivery tumor', 'circadian rhythm cancer treatment'
];
...
async function samplePapers({
  ...
  const seen = new Set();
  const allPapers = [];
  const sourceBreakdown = { openalex: 0, preprints: 0, anomaly: 0, s2: 0, retraction: 0 };
  ...
  // 1. OpenAlex random (60% → ~1620 papers) — primary source
  try {
    const { sampleRandom } = require('./openalex');
    const oaPapers = await sampleRandom(Math.ceil(targetTotal * 0.6));
  ...
  // 2. Fresh preprints (15% → ~405 papers)
  try {
    const { getFreshPreprints } = require('./preprint-monitor');
    const preprints = await getFreshPreprints(7);
  ...
  // 3. OpenAlex anomaly-targeted (10% → ~270 papers)
  try {
    const { sampleByAnomaly } = require('./openalex');
    const anomalyPapers = await sampleByAnomaly(Math.ceil(targetTotal * 0.1));
  ...
  // 4. S2 keyword queries (10% → ~270 papers) — supplement & fallback
  try {
    const s2Papers = await sampleFromS2(Math.ceil(targetTotal * 0.1));
    const added = addPapers(s2Papers, 's2');
    console.log(`[SAMPLER] S2 keywords: ${added} papers`);
    if (onProgress) onProgress(allPapers.length, targetTotal);
  } catch (e) {
    console.error(`[SAMPLER] S2 sampling failed: ${e.message}`);
  }

  // 5. If still under target, try OpenAlex fresh preprints as filler
  if (allPapers.length < targetTotal * 0.7) {
    try {
      const { sampleFreshPreprints } = require('./openalex');
      const freshPapers = await sampleFreshPreprints(200);
  ...
  console.log(`[SAMPLER] Complete: ${allPapers.length} papers | ` +
    `openalex=${sourceBreakdown.openalex} preprints=${sourceBreakdown.preprints} ` +
...
module.exports = { samplePapers, sampleFromS2, SAMPLE_CACHE_FILE };
