...
async function samplePapers({
  ...
  const oaRandomTarget = Math.ceil(targetTotal * 0.6);
  const preprintTarget = Math.ceil(targetTotal * 0.15);
  const anomalyTarget = Math.ceil(targetTotal * 0.1);
  ...
  // 4. If still under target, try OpenAlex fresh preprints as filler
  ...
  // 5. S2 keyword queries — fallback only if still under target
  if (allPapers.length < targetTotal) {
    try {
      const remaining = targetTotal - allPapers.length;
