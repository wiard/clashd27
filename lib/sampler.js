 ...
 *   20% OpenAlex random (diverse fields)
 *   5%  PubMed via Europe PMC
 *   35% bioRxiv/medRxiv/arXiv fresh preprints (AI-heavy)
 *   10% OpenAlex targeted anomaly sampling
 *   10% S2 keyword queries (AI-heavy)
 *   5%  Retraction ecosystem (OpenAlex retracted papers)
 *   10% GitHub trending + watchlist repos (NEW)
 *   5%  Papers With Code cross-reference (NEW)
 *
...
const SOURCE_WEIGHTS = {
  preprints: 0.35,   // AI-heavy preprints (was 0.45)
  openalex: 0.20,    // OpenAlex random diverse fields (was 0.25)
  s2: 0.10,          // S2 keyword queries (AI-heavy)
  anomaly: 0.10,     // OpenAlex targeted anomaly sampling
  github: 0.10,      // GitHub trending + watchlist repos (NEW)
  pubmed: 0.05,      // PubMed via Europe PMC
  retraction: 0.05,  // Retraction ecosystem (OpenAlex retracted papers)
  paperwithcode: 0.05, // Papers With Code cross-reference (NEW)
};
...
async function samplePapers({
  ...
  const seen = new Set();
  const allPapers = [];
  const sourceBreakdown = { openalex: 0, pubmed: 0, preprints: 0, anomaly: 0, s2: 0, retraction: 0, github: 0, paperwithcode: 0 };
  ...
  const githubTarget = Math.ceil(targetTotal * SOURCE_WEIGHTS.github);
  ...
  // 5. GitHub trending + watchlist repos (10%)
  if (process.env.GITHUB_TOKEN && process.env.GITHUB_MONITOR_ENABLED !== 'false') {
    try {
      const { getGitHubProjects } = require('./github-monitor');
      console.log(`[SAMPLER] GitHub start: target ${githubTarget}`);
      const { projects } = await getGitHubProjects({ targetCount: githubTarget });
      const added = addPapers(projects, 'github');
      console.log(`[SAMPLER] GitHub: ${added} projects`);
      if (onProgress) onProgress(allPapers.length, targetTotal);
    } catch (e) {
      console.error(`[SAMPLER] GitHub failed: ${e.message}`);
    }
  } else {
    console.log('[SAMPLER] GitHub skipped (no token or disabled)');
  }

  // 6. If still under target, try OpenAlex fresh preprints as filler
  ...
  // 7. S2 keyword queries — fallback only if still under target
  if (allPapers.length < targetTotal) {
    try {
      const remaining = targetTotal - allPapers.length;
  ...
  console.log(`[SAMPLER] Complete: ${allPapers.length} papers | ` +
    ...
    `anomaly=${sourceBreakdown.anomaly} s2=${sourceBreakdown.s2} github=${sourceBreakdown.github}`);
