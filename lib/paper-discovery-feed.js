...
function confidenceToScore(paper) {
  const cites = Number(paper.citationCount || 0);
  if (cites > 50) return 0.95;
  if (cites > 20) return 0.85;
  if (cites > 5) return 0.7;
  if (cites > 0) return 0.55;
  return 0.35;
}

function buildWhy(paper, cellIndex) {
  const reasons = [];
  const axes = cellAxes(cellIndex);
  const topics = Array.isArray(paper.fieldsOfStudy) ? paper.fieldsOfStudy : [];
  if (topics.length > 1) reasons.push(`cross-domain: ${topics.slice(0, 3).join(", ")}`);
  else if (topics.length === 1) reasons.push(`domain: ${topics[0]}`);
  reasons.push(`mapped to ${axes[0]}/${axes[1]}/${axes[2]}`);
  const cites = Number(paper.citationCount || 0);
  if (cites > 20) reasons.push(`high citation count (${cites})`);
  if (paper.isRetracted) reasons.push("retracted paper — anomaly signal");
  if (paper.year && paper.year >= new Date().getFullYear()) reasons.push("recent publication");
  return reasons;
}
...
function paperToSignalEvent(paper, cellIndex, fallbackTimestamp) {
  ...
  const title = typeof paper.title === "string" && paper.title.trim() ? paper.title.trim() : "untitled paper";
  const topics = Array.isArray(paper.fieldsOfStudy) && paper.fieldsOfStudy.length > 0
    ? paper.fieldsOfStudy.map(f => String(f || "").toLowerCase()).filter(Boolean)
    : ["research"];
  const topic = topics[0] || "paper";
  const publishedYear = paper.year ? new Date(paper.year, 0, 1).toISOString() : null;
  ...
  const score = confidenceToScore(paper);
  ...
  return {
    ...
    source: paperSource,
    externalId: paper.paperId || paper.doi || signalId,
    summary: buildSignalSummary(paper, cellIndex),
    title,
    abstract: typeof paper.abstract === "string" ? paper.abstract.slice(0, 500) : null,
    topics,
    topic,
    publishedAtIso: publishedYear || detectedAtIso,
    ...
    why: buildWhy(paper, cellIndex),
    authors: typeof paper.authors === "string" ? paper.authors : null,
