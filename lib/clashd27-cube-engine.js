...
const WHAT_KEYWORDS = {
  'trust-model': ['consent', 'trust', 'permission', 'safety', 'alignment',
    'evaluation', 'benchmark', 'reward', 'adversarial', 'robustness', 'verification'],
  surface: ['channel', 'api', 'ui', 'mcp', 'tool', 'plugin',
    'function-calling', 'server', 'agent', 'delegation', 'orchestration', 'surface'],
  architecture: ['kernel', 'audit', 'policy', 'knowledge', 'graph',
    'ontology', 'cryptographic', 'protocol', 'runtime', 'governance', 'architecture']
};
...
function computeDomainDistance(cellA, cellB) {
  const a = cellToCoords(cellA);
  const b = cellToCoords(cellB);
  let dist = 0;
  if (a.a !== b.a) dist += 0.1;   // different what-axis
  if (a.b !== b.b) dist += 0.1;   // different where-axis
  if (a.c !== b.c) dist += 0.1;   // different time-axis
  return round(dist, 3);
}
...
// Source quality weights used to scale scoreDelta during ingestion.
// Keys match sourceType values from sourceTypeFromSource().
const SOURCE_QUALITY_WEIGHTS = {
  'paper-theory': 1.5,
  'github-competitor': 1.2,
  'agent-skill': 1.0,
  'internal-system': 0.7
};

// Evidence weight tiers derived from signal metadata.
function computeEvidenceWeight(signal) {
  let w = 1.0;
  // Signals carrying citation counts or explicit confidence get a boost.
  if (signal.citationCount > 0) {
    w += clamp(Math.log2(signal.citationCount + 1) * 0.1, 0, 0.4);
  }
  if (Number.isFinite(signal.evidenceConfidence)) {
    w *= clamp(signal.evidenceConfidence, 0.3, 1.5);
  }
  // Multi-source corroborated evidence (flagged upstream by openclashd).
  if (signal.corroboratedSources > 1) {
    w += clamp((signal.corroboratedSources - 1) * 0.1, 0, 0.3);
  }
  return round(clamp(w, 0.3, 2.0), 3);
}

function computeCellEntropyWeight(cell) {
  const entropySeed = clamp(Number(cell && cell.entropySeed) || 0, 0, 1.9) / 1.9;
  const peerDiversity = clamp((Number(cell && cell.peerDiversity) || 0) / 6, 0, 1);
  const timeSpread = clamp(Number(cell && cell.timeSpread) || 0, 0, 1);
  return round(clamp((entropySeed * 0.45) + (peerDiversity * 0.35) + (timeSpread * 0.20), 0, 1), 3);
}
...
const MAX_PHASE_HISTORY = 108; // 4 full cycles
...
class Clashd27CubeEngine {
  ...
  updateResidue(tick, opts = {}) {
    ...
    if (dt > 0) {
      ...
      // Phase 1: decay — apply to directScore, spilloverScore, evidenceScore separately
      const decayFactor = Math.pow(DECAY_PER_TICK, dt);
      const spilloverDecay = Math.pow(DECAY_PER_TICK * 0.99, dt); // spillover decays faster
      for (let i = 0; i < 27; i += 1) {
        const cell = this.state.cells[String(i)];
        cell.directScore = round((cell.directScore || 0) * decayFactor, 6);
        cell.spilloverScore = round((cell.spilloverScore || 0) * spilloverDecay, 6);
        cell.evidenceScore = round((cell.evidenceScore || 0) * decayFactor, 6);
        cell.score = round(cell.directScore + cell.spilloverScore + cell.evidenceScore, 6);
      }
      ...
      // Record phase snapshot for transition detection
      this._recordPhaseSnapshot(targetTick);
    }
  ...
  _recordPhaseSnapshot(tick) {
    const topo = this.computeTopology();
    const prevPhase = this.state.phaseHistory.length > 0
      ? this.state.phaseHistory[this.state.phaseHistory.length - 1]
      : null;
    ...
    const entry = {
      ...
      entropy: topo.normalizedEntropy,
      activeCells: topo.activeCells,
      totalScore: topo.totalScore,
      dominantWhat: topo.dominantAxes.what.axis,
      dominantWhere: topo.dominantAxes.where.axis,
      dominantTime: topo.dominantAxes.time.axis
    };

    // Detect phase transition
    if (prevPhase && prevPhase.phase !== topo.phase) {
      entry.transition = {
        from: prevPhase.phase,
        to: topo.phase,
        fromTick: prevPhase.tick,
        toTick: tick,
        duration: tick - prevPhase.tick
      };
    }

    this.state.phaseHistory.push(entry);
    if (this.state.phaseHistory.length > MAX_PHASE_HISTORY) {
      this.state.phaseHistory = this.state.phaseHistory.slice(-MAX_PHASE_HISTORY);
    }
  }
  ...
  getPhaseTransitions() {
    return (this.state.phaseHistory || []).filter(e => e.transition);
  }
  ...
  _applyGravity() {
    ...
    // Gravity operates on directScore only (not spillover).
    ...
    for (let i = 0; i < 27; i += 1) {
      const cell = this.state.cells[String(i)];
      cell.directScore = round(clamp((cell.directScore || 0) + deltas[i], 0, 1), 6);
      cell.score = round(clamp(cell.directScore + (cell.spilloverScore || 0) + (cell.evidenceScore || 0), 0, 1), 6);
    }
  }

  _applySpillover(primaryCellId, scoreDelta) {
    // When a signal hits a cell, a fraction spills to face-adjacent neighbors.
    // Spillover is tracked separately and does NOT influence collision detection.
    ...
    for (const nId of neighbors) {
      const neighbor = this.state.cells[String(nId)];
      neighbor.spilloverScore = round(clamp((neighbor.spilloverScore || 0) + spillAmount, 0, 1), 6);
      neighbor.score = round(clamp((neighbor.directScore || 0) + neighbor.spilloverScore + (neighbor.evidenceScore || 0), 0, 1), 6);
    }
  }
  ...
  ingestSignal(rawSignal, opts = {}) {
    ...
    // Phase 3: Source-quality-weighted scoreDelta
    const sourceWeight = SOURCE_QUALITY_WEIGHTS[signal.sourceType] || 1.0;
    const evidenceWeight = computeEvidenceWeight(rawSignal);
    const baseScore = 0.3;
    let scoreDelta = round(baseScore * sourceWeight * evidenceWeight, 6);
    if (sourceChanged) scoreDelta += 0.1;
    if (farApart) scoreDelta += 0.1;
    if (signal.isGapSignal) scoreDelta += 0.2;

    // Track directScore and evidenceScore separately from spillover
    const evidenceContribution = round(scoreDelta * (evidenceWeight - 1.0) * 0.5, 6);
    const directContribution = round(scoreDelta - Math.max(0, evidenceContribution), 6);
    cell.directScore = round(clamp((cell.directScore || 0) + directContribution, 0, 1), 6);
    cell.evidenceScore = round(clamp((cell.evidenceScore || 0) + Math.max(0, evidenceContribution), 0, 1), 6);
    cell.score = round(clamp(cell.directScore + (cell.spilloverScore || 0) + cell.evidenceScore, 0, 1), 6);
    ...
    this.state.signals.push({
      ...
      sourceWeight: round(sourceWeight, 3),
  ...
  detectCollisions(opts = {}) {
    ...
    for (let i = 0; i < 27; i += 1) {
      const cell = this.state.cells[String(i)];
      // Use directScore + evidenceScore for collision eligibility (exclude spillover)
      const truthScore = (cell.directScore || 0) + (cell.evidenceScore || 0);
      if (truthScore > 0) cells.push(cell);
    }

    // Pass 1: Near-field collisions (Manhattan ≤ 1) — original behavior
    for (let i = 0; i < cells.length; i += 1) {
      for (let j = i + 1; j < cells.length; j += 1) {
        ...
        // Use directScore + evidenceScore for combined score (spillover excluded)
        const aScore = (a.directScore || 0) + (a.evidenceScore || 0);
        const bScore = (b.directScore || 0) + (b.evidenceScore || 0);
        const combinedScore = round(aScore + bScore, 3);
        ...
        const entropyWeight = round((computeCellEntropyWeight(a) + computeCellEntropyWeight(b)) / 2, 3);
        const emergenceScore = round((0.46 * density) + (0.28 * sourceFactor) + (0.18 * tickFactor) + (0.08 * entropyWeight), 3);
        ...
        meaningful.push({
          ...
          domainDistance: computeDomainDistance(a.cellId, b.cellId)
        });
      }
    }

    // Pass 2: Far-field collisions (Manhattan 2–3) — new: cross-domain discovery
    for (let i = 0; i < cells.length; i += 1) {
      for (let j = i + 1; j < cells.length; j += 1) {
        const a = cells[i];
        const b = cells[j];
        ...
        const sources = [...new Set([...(a.uniqueSources || []), ...(b.uniqueSources || [])])];
        const sourceTypes = [...new Set([...(a.uniqueSourceTypes || []), ...(b.uniqueSourceTypes || [])])];
        const ticks = unionSorted(a.ticks, b.ticks);
        const aScore = (a.directScore || 0) + (a.evidenceScore || 0);
        const bScore = (b.directScore || 0) + (b.evidenceScore || 0);
        const combinedScore = round(aScore + bScore, 3);

        // Far-field requires stronger evidence
        if (sources.length < 3) continue;
        if (sourceTypes.length < 2) continue;
        if (combinedScore <= 1.2) continue;

        const domDist = computeDomainDistance(a.cellId, b.cellId);
        const density = round(clamp((combinedScore / 2) * 0.7, 0, 1), 3);
        const sourceFactor = clamp((sources.length - 1) / 4, 0, 1);
        const tickFactor = clamp(ticks.length / 6, 0, 1);
        // Far-field gets novelty bonus proportional to domain distance
        const noveltyBonus = domDist * 0.3;
        const entropyWeight = round((computeCellEntropyWeight(a) + computeCellEntropyWeight(b)) / 2, 3);
        const emergenceScore = round((0.34 * density) + (0.22 * sourceFactor) + (0.13 * tickFactor) + (0.19 * noveltyBonus / 0.3) + (0.12 * entropyWeight), 3);
        const id = `col-far-${a.cellId}-${b.cellId}-${hash32(`${sources.join('|')}:${ticks.join(',')}`)}`;
    ...
    // Generate emergence events from both passes
    for (const collision of meaningful) {
      if (collision.emergenceScore > this.emergenceThreshold) {
  ...
  _findClusters(minScore = 0.6) {
    ...
    for (const node of [...active].sort((a, b) => a - b)) {
      ...
      const entropyClusterScore = round(component.reduce((sum, c) => (
        sum + computeCellEntropyWeight(this.state.cells[String(c)])
      ), 0) / component.length, 3);
  ...
  summarizeEmergence(opts = {}) {
    ...
    const transitions = this.getPhaseTransitions();
    if (transitions.length > 0) {
      const latest = transitions[transitions.length - 1];
      suggestions.push(`Last phase transition: ${latest.transition.from} → ${latest.transition.to} at tick ${latest.tick}.`);
    }
