...
class Clashd27CubeEngine {
  ...
  renderAscii(snapshot = null) {
    ...
    for (let z = 0; z < 3; z += 1) {
      ...
      for (let y = 2; y >= 0; y -= 1) {
        ...
        for (let x = 0; x < 3; x += 1) {
          ...
          const inCluster = primaryCluster.has(cellId);
          const inGradient = primaryGradient.has(cellId);
          const marker = inCluster ? '*' : inGradient ? '+' : ' ';
          row.push(`${ch}${marker}`);
        }
        lines.push(row.join(' '));
      }
    ...
    lines.push('Legend: · low  ░ mild  ▒ medium  ▓ high  █ peak  * cluster  + gradient');
    if (snap.gradients.length > 0) {
