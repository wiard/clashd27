...
const { State, ENERGY, loadPack, getActivePack, getCellLabel } = require('./state');
const {
...
const { recordHeat, updateQueueSize } = require('./heat-charge-tracker');
...
const { Clashd27CubeEngine } = require('./clashd27-cube-engine');
const { publishGapProposalHandoffs } = require('./v2-knowledge-publisher');
...
const CLASHD27_STATE_FILE = path.join(DATA_DIR, 'clashd27-cube-state.json');
const DEP_COLLISIONS_FILE = path.join(DATA_DIR, 'dependency-collisions.json');
const SKILL_FILE = path.join(__dirname, '..', 'SKILL.md');
const DOMAIN_REGISTRY_MODULE = path.join(__dirname, '..', 'src', 'domains', 'domain-registry.js');
const SIGNAL_QUEUE_MODULE = path.join(__dirname, '..', 'src', 'queue', 'signal-queue.js');
const DISCOVERY_ORCHESTRATOR_MODULE = path.join(__dirname, '..', 'src', 'orchestration', 'discovery-stream-orchestrator.js');
const GOVERNED_EVENT_EMITTER_MODULE = path.join(__dirname, 'event-emitter.js');

function parseNormalizedThreshold(value, fallback) {
...
const DEEP_DIVE_THRESHOLD = parseNormalizedThreshold(process.env.DEEP_DIVE_THRESHOLD || '0.75', 75);
...
let domainRegistryPromise = null;
let signalQueueModulePromise = null;
let discoveryOrchestratorPromise = null;
let governedEmitterPromise = null;

async function loadDomainRegistryModule() {
  if (!domainRegistryPromise) {
    domainRegistryPromise = import(DOMAIN_REGISTRY_MODULE);
  }
  return domainRegistryPromise;
}

async function loadSignalQueueModule() {
  if (!signalQueueModulePromise) {
    signalQueueModulePromise = import(SIGNAL_QUEUE_MODULE);
  }
  return signalQueueModulePromise;
}

async function loadDiscoveryOrchestratorModule() {
  if (!discoveryOrchestratorPromise) {
    discoveryOrchestratorPromise = import(DISCOVERY_ORCHESTRATOR_MODULE);
  }
  return discoveryOrchestratorPromise;
}

async function loadGovernedEmitterModule() {
  if (!governedEmitterPromise) {
    governedEmitterPromise = import(GOVERNED_EVENT_EMITTER_MODULE);
  }
  return governedEmitterPromise;
}

async function getResearchDomainConfig(domainId) {
  if (!domainId) return null;
  try {
    const registry = await loadDomainRegistryModule();
    return registry.getDomainById ? registry.getDomainById(domainId) : null;
  } catch (err) {
    console.error('[DOMAIN-REGISTRY] Load failed:', err.message);
    return null;
  }
}
...
function countTodayResearchFindings(findings, domainId) {
  const today = new Date().toISOString().slice(0, 10);
  return (findings || []).filter((finding) => {
    if (!finding || !finding.timestamp) return false;
    if (String(finding.timestamp).slice(0, 10) !== today) return false;
    if (finding.type !== 'discovery' && finding.type !== 'draft') return false;
    if (!domainId) return true;
    return finding.domain === domainId;
  }).length;
}
...
// ---------------------------------------------------------------------------
// CLASHD27 semantic cube signal adapters
// ---------------------------------------------------------------------------
...
function extractSignalKeywords(text) {
  ...
  const ordered = ['consent', 'trust', 'permission', 'channel', 'api', 'ui', 'mcp', 'kernel', 'audit', 'policy', 'gap', 'trend', 'anomaly'];
  const out = [];
  for (const kw of ordered) {
    if (src.includes(kw)) out.push(kw);
  }
  return out;
}

function buildSelfObservationSignal(tickNum, activeCell, events) {
  const keywords = ['policy', 'audit', 'channel'];
  const bonds = events.filter(e => e.type === 'bond').length;
  const clashes = events.filter(e => e.type === 'clash').length;
  const deaths = events.filter(e => e.type === 'death').length;
  if (bonds > 0) keywords.push('trust', 'permission');
  if (clashes > 2) keywords.push('trend');
  if (deaths > 0) keywords.push('anomaly', 'gap');
  return {
    id: `self:${tickNum}:${activeCell}`,
    source: 'internal system',
    timestamp: new Date().toISOString(),
    keywords,
    trustLevel: bonds > 0 ? 'high' : 'medium',
    surfaceType: 'engine',
    architectureAspect: 'tick-loop'
  };
}

function buildSkillSignal() {
  try {
    if (!fs.existsSync(SKILL_FILE)) return null;
    const text = fs.readFileSync(SKILL_FILE, 'utf8');
    const stats = fs.statSync(SKILL_FILE);
    const keywords = extractSignalKeywords(text);
    if (keywords.length === 0) keywords.push('policy');
    return {
      id: `skill:${hashSeed(text.slice(0, 2000))}`,
      source: 'ai agent skills',
      timestamp: new Date(stats.mtimeMs || Date.now()).toISOString(),
      keywords,
      trustLevel: 'medium',
      surfaceType: 'skill',
      architectureAspect: 'coordination'
    };
  } catch (e) {
    return null;
  }
...
function buildPaperSignal(activeCell, tickNum, useCube) {
  if (!useCube) return null;
  try {
    const { getCubeDescription } = require('./shuffler');
    const desc = getCubeDescription(activeCell);
    if (!desc || !desc.topPapers || desc.topPapers.length === 0) return null;
    const paper = desc.topPapers[0];
    const year = Number.isFinite(paper.year) ? paper.year : new Date().getUTCFullYear();
    const timestamp = `${year}-01-01T00:00:00.000Z`;
    const text = `${paper.title || ''} ${paper.abstract || ''} ${desc.description || ''}`;
    const keywords = extractSignalKeywords(text);
    if (desc.surpriseLabel && String(desc.surpriseLabel).toLowerCase().includes('anomal')) keywords.push('anomaly');
    if (keywords.length === 0) keywords.push('policy');
    return {
      id: `paper:${paper.paperId || `${activeCell}:${tickNum}`}`,
      source: 'paper/theory',
      timestamp,
      keywords,
      trustLevel: 'medium',
      surfaceType: 'paper',
      architectureAspect: desc.clusterLabel || 'unknown'
    };
  } catch (e) {
    return null;
  }
...
function buildGithubSignal(tickNum) {
  const data = readJSONFile(DEP_COLLISIONS_FILE, null);
  if (!data || !Array.isArray(data.collisions) || data.collisions.length === 0) return null;
  const top = data.collisions[0];
  const keywords = extractSignalKeywords(
    `${top.dependency || ''} ${(top.domains || []).join(' ')} ${(top.repos || []).join(' ')}`
  );
  if (Number(top.score || 0) >= 0.7) keywords.push('gap');
  if (keywords.length === 0) keywords.push('api');
  return {
    id: `github:${top.dependency || 'unknown'}:${tickNum}`,
    source: 'github competitor',
    timestamp: new Date().toISOString(),
    keywords,
    trustLevel: 'medium',
    surfaceType: 'repo',
    architectureAspect: 'dependency'
  };
}

function collectSemanticSignals({ tickNum, activeCell, events, useCube }) {
  const signals = [];
  signals.push(buildSelfObservationSignal(tickNum, activeCell, events));
  const skillSignal = buildSkillSignal();
  if (skillSignal) signals.push(skillSignal);
  const paperSignal = buildPaperSignal(activeCell, tickNum, useCube);
  if (paperSignal) signals.push(paperSignal);
  const githubSignal = buildGithubSignal(tickNum);
  if (githubSignal) signals.push(githubSignal);
...
class TickEngine extends EventEmitter {
  ...
  constructor({ state, tickInterval, useCube } = {}) {
    super();
    this.state = state;
    const envTickIntervalMs = Number(process.env.TICK_INTERVAL_MS);
    this.tickInterval = tickInterval ?? (
      Number.isFinite(envTickIntervalMs)
        ? envTickIntervalMs
        : parseInt(process.env.TICK_INTERVAL || '300', 10) * 1000
    );
    this.useCube = useCube ?? (process.env.USE_CUBE === 'true');
    this.discoveryRuntime = process.env.CLASHD27_DISCOVERY_RUNTIME || 'governed';
    this.legacyResearchEnabled = process.env.CLASHD27_ENABLE_LEGACY_RESEARCH === 'true';
    this.researchDomainId = process.env.RESEARCH_DOMAIN || null;
    this.maxFindingsPerDay = Math.max(1, parseInt(process.env.MAX_FINDINGS_PER_DAY || '50', 10));
    this.minPublishScore = parseNormalizedThreshold(process.env.MIN_PUBLISH_SCORE || '0.80', 80);
    this.minNoveltyScore = parseNormalizedThreshold(process.env.MIN_NOVELTY_SCORE || '0.70', 70);
    this.activeResearchDomain = null;
    this.semanticCube = null;
    this.latestEmergence = null;
    this.latestGovernedDiscovery = null;
    this.latestGapProposalDelivery = null;

    try {
      this.semanticCube = new Clashd27CubeEngine({
        stateFile: CLASHD27_STATE_FILE,
        emergenceThreshold: parseFloat(process.env.CLASHD27_EMERGENCE_THRESHOLD || '0.72')
      });
    } catch (e) {
      console.error('[CLASHD27-INIT ERROR] CubeEngine constructor failed:', e.message, e.stack);
      this.semanticCube = null;
      this._emitError('clashd27-init', e);
    }

    if (this.semanticCube) {
      try {
        this.latestEmergence = this.semanticCube.summarizeEmergence({ persist: false });
      } catch (e) {
        console.error('[CLASHD27-INIT WARN] summarizeEmergence failed (cube preserved):', e.message);
        this.latestEmergence = null;
      }
    }
    ...
    if (!this.legacyResearchEnabled) {
      const suspended = this.deepDiveQueue.length + this.verificationQueue.length + this.validationQueue.length;
      if (suspended > 0) {
        this._log('info', `[LEGACY] Suspended ${suspended} legacy queue item(s); governed discovery runtime is canonical.`);
      }
      this.deepDiveQueue = [];
    ...
    if (this.researchDomainId) {
      loadPack(this.researchDomainId);
    }
    ...
    state.save();
    // Paper ingestion queue + rate-limit
    this._paperQueue = null;
    this._paperLastFetchedAt = 0;
    this._paperFetchIntervalMs = 60 * 60 * 1000; // 1 hour
    this.latestSignalIngestion = null;

    this._log('info', `[DISCOVERY] runtime=${this.discoveryRuntime} legacyResearch=${this.legacyResearchEnabled ? 'enabled' : 'disabled'}`);
  }
  ...
  async _resolveActiveResearchDomain() {
    if (!this.researchDomainId) {
      this.activeResearchDomain = null;
      return null;
    }
    const domain = await getResearchDomainConfig(this.researchDomainId);
    if (domain) {
      this.activeResearchDomain = domain;
      loadPack(domain.id);
    }
    return domain;
  }
  ...
  getClashd27Snapshot() {
    if (this.semanticCube && this.latestEmergence) {
      return {
        clock: this.latestEmergence.clock,
        state: this.semanticCube.getState(),
        snapshot: this.latestEmergence,
        ascii: this.semanticCube.renderAscii(this.latestEmergence)
      };
    }
    return null;
  }
  ...
  /**
   * Fetch external discovery signals, normalize them through the stream
   * boundary, and ingest them into the semantic cube. Rate-limited to once
   * per hour for network-backed paper sources.
   */
  async _ingestPaperSignals(tick) {
    if (process.env.CLASHD27_DISABLE_EXTERNAL_INGEST === '1') {
      this.latestSignalIngestion = {
        papersAnalyzed: 0,
        rawSignalsQueued: 0,
        signalsGenerated: 0,
        normalizedSignals: 0,
    ...
    const now = Date.now();
    if (now - this._paperLastFetchedAt < this._paperFetchIntervalMs) return;

    try {
      if (!this._paperQueue) {
        const signalQueueModule = await loadSignalQueueModule();
        const SignalQueueCtor = signalQueueModule.SignalQueue || signalQueueModule.default?.SignalQueue || signalQueueModule.default;
        this._paperQueue = new SignalQueueCtor();
      }
      const orchestratorModule = await loadDiscoveryOrchestratorModule();
      const runSignalIngestionCycle = orchestratorModule.runSignalIngestionCycle || orchestratorModule.default?.runSignalIngestionCycle;
      const ingestion = await runSignalIngestionCycle({
        queue: this._paperQueue,
        engine: this.semanticCube,
        tick,
        referenceTime: new Date(now).toISOString()
      });

      this._paperLastFetchedAt = now;
      this.latestSignalIngestion = ingestion.report;

      if (ingestion.report.papersAnalyzed > 0 || ingestion.report.normalizedSignals > 0) {
        this._log(
          'info',
          `[DISCOVERY-INGEST] papers=${ingestion.report.papersAnalyzed} raw=${ingestion.report.rawSignalsQueued} generated=${ingestion.report.signalsGenerated} normalized=${ingestion.report.normalizedSignals} dropped=${ingestion.report.droppedSignals} cells=${ingestion.report.cubeCellsActivated}`
        );
      }
    } catch (err) {
      // Non-blocking: log and continue
      this._log('warn', `[PAPERS] ingestion failed: ${err.message}`);
    }
  }

  async runGovernedDiscoveryCycle(opts = {}) {
    if (!this.semanticCube) {
      console.log('[GOVERNED] semanticCube is null — skipping');
      return null;
    }

    const tick = Number.isFinite(opts.tick) ? opts.tick : (this.state.tick || 0);

    // Ingest academic papers before running discovery
    await this._ingestPaperSignals(tick);

    const gatewayUrl = opts.gatewayUrl || process.env.OPENCLASHD_GATEWAY_URL || process.env.OPENCLASHD_V2_URL;
    const token = opts.token || process.env.OPENCLASHD_TOKEN;
    const governedEmitterModule = await loadGovernedEmitterModule();
    const runDiscoveryCycle = governedEmitterModule.runDiscoveryCycle || governedEmitterModule.default?.runDiscoveryCycle;
    const cycle = runDiscoveryCycle(this.semanticCube, {
      tick,
      minGravityScore: opts.minGravityScore,
      maxHotspots: opts.maxHotspots,
      systemEntropy: opts.systemEntropy
    });
    console.log('[GOVERNED] cycle result:', JSON.stringify({
      packets: cycle?.gapDiscovery?.packets?.length || cycle?.discovery?.gapPackets?.length || 0,
      handoffs: cycle?.gapProposalHandoffs?.length || cycle?.discovery?.gapProposalHandoffs?.length || 0,
      candidates: cycle?.discovery?.candidates?.length || 0
    }));
    ...
    console.log('[GOVERNED] delivery check:', {
      hasGatewayUrl: !!gatewayUrl,
      hasToken: !!token,
      handoffCount: cycle?.gapProposalHandoffs?.length || 0
    });

    if (!gatewayUrl || !token) {
      console.warn('[GOVERNED] WARNING: no gateway URL or token — handoffs will not be delivered');
      if (opts.deliver !== false && Array.isArray(cycle.gapProposalHandoffs) && cycle.gapProposalHandoffs.length > 0) {
        delivery = {
          published: 0,
          deduped: 0,
          skipped: cycle.gapProposalHandoffs.length,
          failed: 0,
          results: cycle.gapProposalHandoffs.map((handoff) => ({
            published: false,
            skipped: true,
    ...
    if (opts.deliver !== false && gatewayUrl && token && Array.isArray(cycle.gapProposalHandoffs) && cycle.gapProposalHandoffs.length > 0) {
      try {
        delivery = await publishGapProposalHandoffs(cycle.gapProposalHandoffs, {
          gatewayUrl,
          token,
        ...
        console.error('[GOVERNED] delivery error:', err);
        delivery = {
          published: 0,
          deduped: 0,
          skipped: 0,
          failed: Array.isArray(cycle.gapProposalHandoffs) ? cycle.gapProposalHandoffs.length : 1,
          results: [{
            published: false,
            failed: true,
            reason: err.message
    ...
    console.log('[GOVERNED] delivery result:', JSON.stringify(delivery));

    this.latestGovernedDiscovery = cycle;
    this.latestGapProposalDelivery = delivery;
    this.emit('governedDiscovery', { tick, discovery: cycle, delivery });
    this.emit('gapHandoffDelivery', {
      tick,
      delivery,
      handoffs: cycle.gapProposalHandoffs || []
    });

    return {
      ...cycle,
      ingestion: this.latestSignalIngestion || null,
      delivery
    };
  }

  _updateClashd27Cube(tickNum, activeCell, events) {
    if (!this.semanticCube) return;
    try {
      this.semanticCube.updateResidue(tickNum, { persist: false });
      const signals = collectSemanticSignals({
        tickNum,
        activeCell,
        events,
        useCube: this.useCube
      });

      // Cross-layer resonance: bonds at high-residue cells create amplification signals
      const bondEvents = events.filter(e => e.type === 'bond');
      for (const bond of bondEvents) {
        const cell = this.semanticCube.getState().cells[String(bond.cell)];
        if (cell && cell.score >= 0.4) {
          signals.push({
            id: `resonance:${tickNum}:${bond.cell}:${bond.agent1}:${bond.agent2}`,
            source: 'internal system',
            timestamp: new Date().toISOString(),
            keywords: ['trust', 'permission', bond.crossLayer ? 'gap' : 'channel'],
            trustLevel: 'high',
            surfaceType: 'arena',
            architectureAspect: 'coordination'
          });
      ...
      for (const signal of signals) {
        this.semanticCube.ingestSignal(signal, {
          tick: tickNum,
          persist: false,
          referenceTime: ref
        });
      }
      this.latestEmergence = this.semanticCube.summarizeEmergence({ tick: tickNum, persist: false });
      this.semanticCube.save();

      // Discovery triggers: emit emergenceTrigger for high-emergence collisions
      // that the discovery pipeline can act on.
      const newTriggers = this._checkEmergenceTriggers(tickNum, this.latestEmergence);
      ...
      this._emitError('clashd27-update', e, { tick: tickNum, activeCell });
    }
  }

  _checkEmergenceTriggers(tickNum, emergence) {
    if (!emergence) return [];
    const triggers = [];
    const emittedKey = `_emittedTriggers`;
    if (!this[emittedKey]) this[emittedKey] = new Set();

    // Trigger 1: High-emergence collisions above threshold
    for (const collision of (emergence.collisions || []).slice(0, 5)) {
      if (collision.emergenceScore <= this.semanticCube.emergenceThreshold) continue;
      const key = `col:${collision.cells.join('-')}`;
      if (this[emittedKey].has(key)) continue;
      this[emittedKey].add(key);
      const cellA = this.semanticCube.getState().cells[String(collision.cells[0])];
      const cellB = this.semanticCube.getState().cells[String(collision.cells[1])];
      const trigger = {
        type: 'emergence-collision',
        id: `trigger-${collision.id}`,
        tick: tickNum,
        cells: collision.cells,
        axesA: cellA.axes,
        axesB: cellB.axes,
        emergenceScore: collision.emergenceScore,
        sources: collision.sources,
        hypothesis: `Semantic collision between ${cellA.axes.what}/${cellA.axes.where}/${cellA.axes.time} and ${cellB.axes.what}/${cellB.axes.where}/${cellB.axes.time} — emergence score ${collision.emergenceScore}.`
      };
      triggers.push(trigger);
      this.emit('emergenceTrigger', trigger);
    }

    // Trigger 2: Strong gravity wells with high momentum (actively heating)
    for (const well of (emergence.gravityWells || []).slice(0, 3)) {
      if (well.gravityMass < 0.8) continue;
      const mEntry = (emergence.momentum || []).find(m => m.cellId === well.cellId);
      if (!mEntry || mEntry.trend !== 'heating') continue;
      const key = `well:${well.cellId}`;
      if (this[emittedKey].has(key)) continue;
      this[emittedKey].add(key);
      const trigger = {
        type: 'gravity-well',
        id: `trigger-well-${well.cellId}-${tickNum}`,
        tick: tickNum,
        cellId: well.cellId,
        axes: well.axes,
        gravityMass: well.gravityMass,
        momentum: mEntry.momentum,
        hypothesis: `Active gravity well at ${well.axes.what}/${well.axes.where}/${well.axes.time} — mass ${well.gravityMass}, momentum ${mEntry.momentum}.`
      };
      triggers.push(trigger);
      this.emit('emergenceTrigger', trigger);
    }

    // Cap emitted set to prevent unbounded growth
    if (this[emittedKey].size > 500) {
      const arr = [...this[emittedKey]];
      this[emittedKey] = new Set(arr.slice(-250));
    }
  ...
  _saveQueues() {
    try {
      updateQueueSize(this.deepDiveQueue.length + this.verificationQueue.length + this.validationQueue.length);
  ...
  async _tickInner() {
    ...
    const activeResearchDomain = await this._resolveActiveResearchDomain();
    ...
    this._updateClashd27Cube(tickNum, activeCell, events);
    ...
    if (isCycleEnd) {
      ...
      if (this.latestEmergence) {
        summary.clashd27 = {
          strongestCell: this.latestEmergence.strongestCell,
          topRoutes: (this.latestEmergence.topRoutes || []).slice(0, 3),
          clusters: (this.latestEmergence.clusters || []).slice(0, 3),
          gravityWells: (this.latestEmergence.gravityWells || []).slice(0, 3),
          momentum: (this.latestEmergence.momentum || []).slice(0, 5),
          optimalRoutes: (this.latestEmergence.optimalRoutes || []).slice(0, 3)
        };
      }
      try {
        const governed = await this.runGovernedDiscoveryCycle({ tick: tickNum });
        summary.gapDiscovery = {
          packetCount: governed?.gapDiscovery?.packets?.length || 0,
          handoffCount: governed?.gapProposalHandoffs?.length || 0,
          delivered: governed?.delivery?.published || 0,
          deduped: governed?.delivery?.deduped || 0
        };
      } catch (err) {
        this._log('error', `[DISCOVERY] Governed cycle failed: ${err.message}`);
        this._emitError('governed-discovery', err, { tick: tickNum });
      }
    ...
    const packName = pack ? pack.name : (activeResearchDomain?.label || 'Research');
    const domainKeywords = mergeKeywords(activeResearchDomain?.keywords || []);
    const todaysFindings = countTodayResearchFindings(readFindings().findings || [], this.researchDomainId);
    const findingsQuotaReached = todaysFindings >= this.maxFindingsPerDay;

    if (this.legacyResearchEnabled && agentsHere.length > 0 && pack && !budgetPaused && !findingsQuotaReached) {
        ...
        if (agent1 && agent2 && agent1.homeCell !== agent2.homeCell) {
          ...
          if (priorDiscoveries.length >= 2) {
            ...
            try {
              const screenResult = await screenCollision(
                { label: dataLabel, keywords: mergeKeywords(getAgentKeywords(dataAgent), domainKeywords) },
                { label: hypoLabel, keywords: mergeKeywords(getAgentKeywords(hypoAgent), domainKeywords) },
            ...
            try {
              const discovery = await withTimeout(investigateDiscovery({
                ...
                packName,
                dataKeywords: mergeKeywords(getAgentKeywords(dataAgent), domainKeywords),
                hypothesisKeywords: mergeKeywords(getAgentKeywords(hypoAgent), domainKeywords),
                priorSummary,
              ...
              if (discovery) {
                if (discovery.type === 'no_gap') {
                  ...
                  if (discovery.type === 'discovery' && totalScore >= DEEP_DIVE_THRESHOLD) {
          ...
          try {
            const bondFinding = await withTimeout(investigateBond({
              ...
              cell2Label: a2Label,
              packName,
              cell1Keywords: mergeKeywords(getAgentKeywords(agent1), domainKeywords),
              cell2Keywords: mergeKeywords(getAgentKeywords(agent2), domainKeywords)
      ...
      state.save();
    } else if (this.legacyResearchEnabled && findingsQuotaReached) {
      this._log('info', `[RESEARCH] Daily finding cap reached for ${this.researchDomainId || 'default'} (${todaysFindings}/${this.maxFindingsPerDay})`);
    }

    // === DEEP DIVE (max 1 per tick, with timeout) ===
    if (this.legacyResearchEnabled && this.deepDiveQueue.length > 0 && !lowSpendMode && !budgetPaused && (Date.now() - this._tickStartedAt) < TICK_TIME_BUDGET_MS) {
    ...
    // === VERIFICATION ===
    if (this.legacyResearchEnabled && this.verificationQueue.length > 0 && !lowSpendMode && !budgetPaused && (Date.now() - this._tickStartedAt) < TICK_TIME_BUDGET_MS) {
    ...
    // === VALIDATION ===
    if (this.legacyResearchEnabled && this.validationQueue.length > 0 && !lowSpendMode && !budgetPaused && (Date.now() - this._tickStartedAt) < TICK_TIME_BUDGET_MS) {
    ...
    if (this.legacyResearchEnabled) {
      // === PUBLISH HIGH-VALUE DISCOVERIES TO OPENCLASHD-V2 ===
      try {
        const gatewayUrl = process.env.OPENCLASHD_GATEWAY_URL;
        const token = process.env.OPENCLASHD_TOKEN;
        if (gatewayUrl && token) {
          const findingsData = readFindings();
          const allFindings = findingsData.findings || [];
          const tickFindings = allFindings.filter(f => f.tick === tickNum && f.type === 'discovery');
          if (tickFindings.length > 0) {
            const { publishToV2, publishExtensionProposals } = require('./v2-knowledge-publisher');
            const { generateExtensionCandidates } = require('./extension-candidates');

            publishToV2(tickFindings, { gatewayUrl, token }).then(publishResult => {
              if (publishResult.published > 0) {
                this._log('info', `[clashd27] published ${publishResult.published} discoveries to v2`);
              }
            }).catch(err => {
              this._log('warn', `[clashd27] v2 publish failed: ${err.message}`);
            });

            try {
              const extensionCandidates = generateExtensionCandidates(tickFindings, {
                allFindings,
                tick: tickNum
              });
              if (extensionCandidates.length > 0) {
                publishExtensionProposals(extensionCandidates, { gatewayUrl, token }).then(result => {
                  if (result.published > 0) {
                    this._log('info', `[clashd27] published ${result.published} extension proposals to v2`);
                  }
                }).catch(err => {
                  this._log('warn', `[clashd27] extension publish failed: ${err.message}`);
                });
              }
            } catch (err) {
              this._log('warn', `[clashd27] extension candidate generation failed: ${err.message}`);
            }
          }
        }
      } catch (err) {
      ...
      // === DAILY GAP PUBLISH ===
      try {
        const pub = publishDailyGapsIfNeeded({
          maxGaps: 5,
          minPublishScore: this.minPublishScore,
          minNoveltyScore: this.minNoveltyScore
        });
        if (pub.published && pub.published > 0) {
          this._log('info', `[PUBLISH] Added ${pub.published} gaps (total today: ${pub.total})`);
        }
        if (pub.drafted && pub.drafted > 0) {
          this._log('info', `[PUBLISH] Deferred ${pub.drafted} findings to drafts (quality threshold)`);
        }
      } catch (e) {
...
if (require.main === module) {
  ...
  if (domainIndex !== -1 && args[domainIndex + 1]) {
    process.env.RESEARCH_DOMAIN = args[domainIndex + 1];
  }
  if (!process.env.CLASHD27_ENABLE_LEGACY_RESEARCH) {
    process.env.CLASHD27_ENABLE_LEGACY_RESEARCH = 'true';
  }
  ...
  engine.on('log', ({ level, msg, ctx }) => {
    ...
    if (ctx) writer(`${msg} ${JSON.stringify(ctx)}`);
    else writer(msg);
  });

  engine.on('error', ({ phase, error, ctx }) => {
    if (ctx) {
      console.error(`[${phase}] ${error.message} ${JSON.stringify(ctx)}`);
    } else {
  ...
  if (once) {
    engine._tick().then(() => shutdown(0)).catch((err) => {
      console.error(`[ENGINE] once run failed: ${err.message}`);
      shutdown(1);
    });
  } else {
    console.log(`[ENGINE] CLASHD-27 tick engine starting`);
    console.log(`[ENGINE] Domain: ${process.env.RESEARCH_DOMAIN || 'default'}`);
    console.log(`[ENGINE] Tick interval: ${engine.tickInterval}ms`);
    engine.start();
