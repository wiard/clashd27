...
class TickEngine extends EventEmitter {
  ...
  constructor({ state, tickInterval, useCube } = {}) {
    ...
    if (this.deepDiveQueue.length || this.verificationQueue.length || this.validationQueue.length) {
      console.log(`[QUEUES] Restored: deepDive=${this.deepDiveQueue.length}, verification=${this.verificationQueue.length}, validation=${this.validationQueue.length}`);
    }

    // Collision cache (per cube generation)
    this.collisionCache = new Set();
    this.collisionCacheGeneration = null;
  ...
  start() {
    ...
    console.log(`[ENGINE] Tick engine started (interval=${this.tickInterval / 1000}s)`);
  }
  ...
  stop() {
    ...
    this.state.save();
    this._saveQueues();
    console.log(`[ENGINE] Tick engine stopped, state saved.`);
  }
  ...
  _saveQueues() {
    try {
      ...
      console.error(`[QUEUES] Save failed: ${e.message}`);
    }
  }
  ...
  // -----------------------------------------------------------------------
  async _tick() {
    if (this._tickRunning) { console.log('[TICK] SKIP overlap'); return; }
    ...
    try {
      ...
      this.emit('error', { phase: 'tick', error: err, context: { tick: this.state.tick } });
    } finally {
      this._tickRunning = false;
      releaseTickLock();
    }
  ...
  async _tickInner() {
    ...
    if (isCycleEnd) {
      this.emit('cycleEnd', { summary: state.getCycleSummary() });
    }
      ...
      try {
        ...
        if (shouldShuffle(tickNum, lastShuffle) && !global._shuffleInProgress) {
          ...
          shuffle(tickNum, gen, (fetched, total) => {
            ...
            this.emit('error', { phase: 'shuffle', error: err });
            global._shuffleInProgress = false;
          });
        }
      } catch (err) {
    ...
    if (agentsHere.length > 0 && pack) {
      ...
      if (agent1 && agent2 && agent1.homeCell !== agent2.homeCell) {
        ...
        if (crossLayer) {
          ...
          if (priorDiscoveries.length >= 2) {
            ...
            if (claimData && !isLegacyDiscovery(claimData.discovery)) {
              ...
              try {
                ...
                this.emit('error', { phase: 'claim-verify', error: err });
              }
            }
          } else {
            ...
            try {
              const { goldenCollisionScore, getCubeDescription, readCube } = require('./shuffler');
              const cube = readCube();
              const cubeGen = cube ? cube.generation : null;
              if (cubeGen !== this.collisionCacheGeneration) {
                this.collisionCacheGeneration = cubeGen;
                this.collisionCache = new Set();
              }
              if (cubeGen !== null) {
                ...
                if (this.collisionCache.has(collisionKey)) {
                ...
                this.collisionCache.add(collisionKey);
              }
              ...
              this.emit('error', { phase: 'golden-scoring', error: e });
            }
            ...
            try {
                ...
                if (discovery.type === 'no_gap') {
                  ...
                  if (discovery.type === 'discovery' && verdictStr === 'HIGH-VALUE GAP') {
                    try {
                      ...
                      this.emit('error', { phase: 'saturation', error: satErr, context: { discoveryId: discovery.id } });
                    }
                  }
                  ...
                  if (discovery.type === 'discovery' && totalScore >= DEEP_DIVE_THRESHOLD && (verdictStr === 'HIGH-VALUE GAP' || discovery.impact === 'high')) {
                    ...
                    this._saveQueues();
                    console.log(`[DEEP-DIVE] Queued ${discovery.id} for deep dive (queue: ${this.deepDiveQueue.length})`);
              ...
              this.emit('error', { phase: 'discovery', error: err });
          ...
          try {
            ...
            this.emit('error', { phase: 'bond', error: err });
          }
        }
      }
      ...
      if (!apiUsed) {
        ...
        try {
          ...
          this.emit('error', { phase: 'cell', error: err });
        }
      }
    ...
    if (this.deepDiveQueue.length > 0 && !lowSpendMode && (Date.now() - this._tickStartedAt) < TICK_TIME_BUDGET_MS) {
      ...
      if (!budget.canRunDeepDive(MAX_DEEP_DIVES_PER_DAY)) {
        ...
        console.log(`[DEEP-DIVE] Starting deep dive on ${ddDiscovery.id} (${this.deepDiveQueue.length} remaining)`);
        try {
          ...
          if (ddResult) {
            ...
            if (ddResult.verdict === 'HIGH-VALUE GAP') {
              ...
              this._saveQueues();
              console.log(`[VERIFIER] QUEUED ${ddDiscovery.id} for GPT-4o adversarial review (queue: ${this.verificationQueue.length})`);
            }
          } else {
          ...
          this.emit('error', { phase: 'deep-dive', error: err });
    ...
    // === VERIFICATION ===
    if (this.verificationQueue.length > 0 && !lowSpendMode && (Date.now() - this._tickStartedAt) < TICK_TIME_BUDGET_MS) {
          ...
          try {
            ...
            if (vResult && !vResult.error) {
              ...
              if (vResult.survives_scrutiny && vResult.gpt_verdict === 'CONFIRMED') {
                ...
                this._saveQueues();
                console.log(`[VALIDATOR] Queued ${vItem.discovery.id} for pre-experiment validation (queue: ${this.validationQueue.length})`);
              }
              ...
              this.emit('error', { phase: 'verification', error: new Error(vResult.error), context: { category: cat } });
            }
            // null means rate-limited — keep in queue
          } catch (err) {
            console.error(`[VERIFIER] Error: ${err.message}`);
            this.emit('error', { phase: 'verification', error: err });
    ...
    // === VALIDATION ===
    if (this.validationQueue.length > 0 && !lowSpendMode && (Date.now() - this._tickStartedAt) < TICK_TIME_BUDGET_MS) {
      ...
      try {
        ...
        this.emit('error', { phase: 'validation', error: err });
        this.validationQueue.shift();
        this._saveQueues();
      }
    }
